...
> - 并发(Concurrent): 等待时间可以开始下一个任务; 耗时:  5+2+10=17(
    C任务10分钟大于B任务8分钟,且B等待2分钟+C任务10分钟大于A任务等待10分钟)
...
- IO操作不占用cpu，只要我们一般拷贝文件使用的是[阻塞IO]
  ，这时相当于线程虽然不用cpu，但需要一直等待IO结束，没有充分利用线程。所以有了后面的[非阻塞IO]和[异步IO]优化
...
> -Dcom.sun.management.jmxremote.ssl=是否安全连接 -Dcom.sun.management.jmxremote.authenticate=是否认证Java类
>
...
> 当Context Switch 发生时，需要由操作系统保存当前线程的状态，并恢复另一个线程的状态，Java中对应的概念就是程序计数器(Program
> Counter Register)，它的作用是记住下一条jvm指令的执行地址，是线程私有的
...
- NEW 线程刚刚被创建，但是还没有调用start()方法
    - RUNNABLE当调用start()方法之后，注意，**Java API** 
  层面的 *RUNNABLE*
      状态覆盖了操作系统层面的【可运行状态】、
     【运行状态】和【阻塞状态】（由于BIO读取导致的线程
      阻塞，在Java里无法区分，仍然认为是可运行）
- TERMINATED当代码运行结束
...
- 这中间即使t1的cpu时间片不幸用完，被踢出了门外(不要错误理解为锁住了对象就能一直执行下去哦)
  ，这时门还是锁住的，t1仍拿着钥匙,t2线程还在阻塞状态进不来，只有下次轮到t1自己再次获得时间片时才能开门进入
- 当t1执行完 synchronized{}块内的代码，这时候才会从 obj房间出来并解开门上的锁，唤醒t2线程把钥匙给他。t2线程这时才可以进入
  obj房间，锁住了门拿上钥匙,执行它的count--代码
...
public class MyAspect {
    //线程不安全，Spring默认注入对象是单例的，那他就会存在共享问题，也就存在线程安全问题
...
public class UserServiceImpl implements UserService {
    //userDao是线程安全的，因为uerDao中没有属性可以修改，没有共享变量，线程安全
...
public class UserDaoImpl implements UserDao {
    //存在线程安全问题，可能一个线程new Connection，另一个线程赋值为null或者执行了close()方法，第一个线程就会空指针异常
...
- Thread-2执行完同步代码块的内容，然后唤醒EntryList 中等待的线程来竞争锁，竞争的时候是非公平的
...
- 当前线程等待的时间超过了n毫秒后，或t线程运行结束后，或调用当前线程的interrupt()方法时，当前线程从 TIMED_WAITING --->
  RUNNABLE
...
- 调用LockSupport.unpark(目标线程)或调用了线程的interrupt()方法，或是等待的时间超时，会让目标线程从 TIMED_WAITING --->
  RUNNABLE
...
- 持obj锁线程的同步代码块执行完毕，会唤醒该对象上的所有BLOCKED线程重新竞争，如果其中t线程竞争成功，从BLOCKED ---> RUNNABLE
  ，其他竞争失败的线程仍然是，BLOCKED
...
happens-before规定了对共享变量的写操作对其他线程的读操作可见，它是可见性和有序性的一套规则总结，抛开以下happens-before规则，JMM并不能保证一个线程对共享变量的写，对于其他线程对该共享变量的读可见
...
- 线程结束前对变量的写，对其他线程得知他结束以后的可见性（比如其他先调用t1.isLive()或t1.join等待他结束）
...
- 具有传递性，如果x hb->y并且y hb->z，配合volatile的防止指令重排，有下面例子
```java
class Test {
    volatile static int x;
    static int y;
...
### 习题
#### balking模式练习
希望doInit()方法只被调用一次，下面的实现是否有问题，为什么
```java
...
> 
### 线程安全单例练习题
单例模式有很多实现方法，饿汉，懒汉，静态内部类，枚举类，思考下面问题
> 饿汉式：类加载就会直接把单例对象创建
> 
> 懒汉式：类加载的时候不会将单例对象创建，而是在使用该对象才会创建
> 
实现1：
```java
@Slf4j
...
```
> 问题1：防止子类继承，而覆盖父类的方法，破坏单例
>
> 问题2：可见代码
> 
> 问题3：不设为private，外部类就可以无限创建对象，破坏了单例；不能防止反射创建对象
> 
> 问题4：安全，类加载的过程就会加载，且这个（类加载对象）类对象只会有一份，我们可以完全相信jvm
> 
> 问题5：由方法提供
> - 以实现更好的封装性，还可以提供懒惰的初始化
> - 获取这个单例对象时，可以在方法中加以控制
> - 提供一些泛型的支持
> 
实现2：
```java
//问题1:枚举单例是如何限制实例个数的
// 问题2:枚举单例在创建时是否有并发问题l
// 问题3:枚举单例能否被反射破坏单例
// 问题4:枚举单例能否被反序列化破坏单例
// 问题5:枚举单例属于懒汉式还是饿汉式
...
```
> 问题1：枚举对象你定义时有几个，将来就有几个对象，相当于枚举类的静态成员变量
> 
> 问题2：没有，他是静态成员变量，在类加载的时候创建的，不会有并发问题
> 
> 问题3：不能通过反射破坏单例
> 
> 问题4：不会被破坏
...
```
> 第一次调用会加锁，但后面的调用其实没有比较加锁，所以效率不高
> 
实现4：
```java
public final class Singleton {
...
```
> 问题1：保证可见性，可读性和禁止重排序，INSTANCE只有这一个地方修改，其他线程只需要读取，加上volatile保证可读性和可见性
> 
> 问题3：有可能会有多个线程执行到synchronized()加锁的步骤，只需要第一个拿到锁的人创建对象，其他人直接返回INSTANCE就行了
> 
实现5：静态内部类实现
```java
public final class Singleton {
